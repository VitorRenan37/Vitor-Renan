...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE vitor1d SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM tabela_vitor WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM vitor1d WHERE id=$editar_id");
...
<?php
$seleciona = mysqli_query($conexao, "SELECT * FROM tabela_vitor ORDER BY id DESC");
